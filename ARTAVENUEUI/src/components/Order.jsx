...
const Order = () => {
  ...
  const [fetchItemNow, setFetchItemNow] = useState({ images: [] });
...
useEffect(() => {
  ...
  return (
    ...
    <CustomCarousel>
        {fetchItemNow.images.map((image, index) => {
          return <img key={index} src={transformImagePath(image.photoFormat)} alt={image.caption} />;
        })}
      </CustomCarousel>
      ...
      <div class="mt-4 mb-10 px-6 lg:px-14">
        ...
        <div class="mt-6 flex flex-col items-center">
          <p class="mt-1 text-sm text-gray-400">Description: {fetchItemNow?.description}</p>
        </div>
        <div class="mx-auto mt-8 flex w-60 flex-col space-y-4">
          <div class="flex items-center space-x-3">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="inline h-7 w-7 rounded-full bg-blue-100 p-1 text-blue-500">
              <path stroke-linecap="round" stroke-linejoin="round" d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 11.953 0 0112 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0121 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0112 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 013 12c0-1.605.42-3.113 1.157-4.418"></path>
            </svg>
            <span class="text-sm text-gray-500">Category: {fetchItemNow?.categoryName}</span>
          </div>
          <div class="flex items-center space-x-3">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="inline h-7 w-7 rounded-full bg-blue-100 p-1 text-blue-500">
            ...
            <span class="text-sm text-gray-500">Artist: {fetchItemNow?.firstArtist}</span>
          </div>
          ...
          <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5m-9-6h.008v.008H12v-.008ZM12 15h.008v.008H12V15Zm0 2.25h.008v.008H12v-.008ZM9.75 15h.008v.008H9.75V15Zm0 2.25h.008v.008H9.75v-.008ZM7.5 15h.008v.008H7.5V15Zm0 2.25h.008v.008H7.5v-.008Zm6.75-4.5h.008v.008h-.008v-.008Zm0 2.25h.008v.008h-.008V15Zm0 2.25h.008v.008h-.008v-.008Zm2.25-4.5h.008v.008H16.5v-.008Zm0 2.25h.008v.008H16.5V15Z" />
            ...
            <span class="text-sm text-gray-500">Year: {fetchItemNow?.year}</span>
          </div>
          ...
          <div class="flex items-center space-x-3">
            ...
            <span class="text-sm text-gray-500">Note: {fetchItemNow?.note}</span>
          </div>
        </div>
      </div>
      </div>
        ...
        />
          ...
          <hr className='hrline'/>
          <div className='price'>
  <p>{isFromBuyComponent ? 'Price:' : 'Start Price:'}</p>
  {isFromBuyComponent ? (fetchItemNow.buyimmediatelyPrice +'€' || '') : (fetchItemNow.startPrice + '€' ||fetchItemNow.startPrice)}
</div>
        
       
          <button
        className={`add-to-bag-button ${!isLoggedInClient ? 'disabled-button' : ''}`}
  ...
  {isOpen ? 'Hide Total with Taxes and Fees' : 'Show Total with Taxes and Fees'}
</button>
      <div className={`breakdown-section ${isOpen ? 'show' : 'hide'}`}>
        <div className="breakdown-item">
          <span><span style={{color:'#f3eee2'}}> <PaymentIcon/></span>  <strong> My Price:</strong></span>
          <span>€{validBid.toFixed(2)}</span>
        </div>
        <div className="breakdown-item">
          <span><span style={{color:'#f3eee2 ',fontFamily: 'Roboto, sans-serif'}}><FactCheckIcon/></span> <strong> Tax (18%):</strong></span>
          <span>€{tax.toFixed(2)}</span>
        </div>
        <div className="breakdown-item">
          <span> <span style={{color:'#f3eee2 '}}><FactCheckIcon/></span><strong> BidBrush Fee (3%):</strong></span>
          <span>€{bidBrushFee.toFixed(2)}</span>
        </div>
        <hr className="breakdown-separator" />
        <div className="breakdown-item breakdown-total">
          <span> <span style={{color:'#f3eee2'}}><ShoppingCartIcon/></span><strong>Total:</strong></span>
          <span>€{total.toFixed(2)}</span>
        </div>
      </div>
          <div style={{marginTop:'auto'}} className='unit'>
          <div class="my-10 max-w-xs rounded-xl bg-pink-100 px-6 py-8 text-pink-800">
  <p class="mb-2 text-2xl font-medium">Dimensions</p>
  <p class="mb-6">The dimensions for this art item are in <b>{fetchItemNow.unit}</b> .</p>
